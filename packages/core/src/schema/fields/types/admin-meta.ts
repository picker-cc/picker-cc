...
export type FieldController<FormState, FilterValue extends JSONValue = never> = {
    ...
    filter?: {
        ...
        Label(type: FilterTypeToFormat<FilterValue>): string | null;
        Filter(props: {
            type: string;
            value: FilterValue;
...
export type FieldViews = Record<
    string,
    {
        Field: (props: FieldProps<any>) => any | null;
        Cell: CellComponent;
...
export type CardValueComponent<
    FieldControllerFn extends (...args: any) => FieldController<any, any> = () => FieldController<
        ...
        any
        >
    > = (props: { item: Record<string, any>; field: ReturnType<FieldControllerFn> }) => any;

export type ContextFunction<Return> = (context: PickerContext) => MaybePromise<Return>;
