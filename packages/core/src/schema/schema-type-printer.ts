import {introspectionTypes} from "graphql/type";
import {
    GraphQLEnumType,
...
function printListTypeInfo<L extends InitialisedList>(listKey: string, list: L) {
    ...
    // prettier-ignore
    return [
        `export type ${listKey} = import('@picker-cc/core').ListConfig<${listTypeInfoName}, any>;`,
        `namespace ${listKey} {`,
...
export function printGeneratedTypes(
    ...
    return [
        printInputTypesFromSchema(graphQLSchema, {
            ...
            Int: 'number',
            Float: 'number',
            JSON: 'import("@picker-cc/core/types").JSONValue',
            Decimal: 'import("@picker-cc/core/types").Decimal | string',
        }),
        '',
        ...
        ``,
        `export type Models = {`,
        `  [Key in keyof TypeInfo['lists']]?: import('@picker-cc/core').ListConfig<TypeInfo['lists'][Key], any>`,
        `} & Record<string, import('@picker-cc/core').ListConfig<any, any>>;`,
        ``,
        `export {}`,
        ``,
    ].join('\n');
