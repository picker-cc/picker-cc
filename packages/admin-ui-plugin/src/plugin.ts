...
import { AdminUiConfig, AdminUiExtension, Type } from '@picker-cc/common/lib/shared-types';
import {
    AuthOptions,
    ConfigService,
    createProxyHandler,
...
export interface AdminUiOptions {
    ...
    /**
     * @description
     * The hostname of the Picker server which the admin ui will be making API calls
    ...
    /**
     * @description
     * The port of the Picker server which the admin ui will be making API calls
...
/**
 * @description
 * This plugin starts a static server for the Admin UI app, and proxies it via the `/admin/` path of the main Picker server.
 *
 ...
 * ## Installation
 *
 * `yarn add \@Picker/admin-ui-plugin`
 *
 * or
 *
 * `npm install \@Picker/admin-ui-plugin`
 *
 * @example
 * ```ts
 * import { AdminUiPlugin } from '\@Picker/admin-ui-plugin';
 *
...
export class AdminUiPlugin implements OnPickerBootstrap, OnPickerClose {
    ...
    /** @internal */
    static async configure(config: RuntimePickerConfig): Promise<RuntimePickerConfig> {
        const route = 'admin';
    ...
    async onPickerBootstrap() {
        ...
        if (watch) {
            ...
            adminUiConfigPath = path.join(__dirname, '../../../admin-ui/src', 'picker-ui-config.json');
        } else {
            ...
            adminUiConfigPath = path.join(UI_PATH, 'picker-ui-config.json');
        }
    ...
    /** @internal */
    async onPickerClose(): Promise<void> {
        if (this.watcher) {
            this.watcher.close();
        }
        if (this.server) {
    ...
    /**
     * Overwrites the parts of the admin-ui app's `Picker-ui-config.json` file relating to connecting to
