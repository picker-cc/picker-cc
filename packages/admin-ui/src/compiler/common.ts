...
import {AdminUiExtension, AdminUiExtensionModule} from '@picker-cc/common/lib/shared-types';
...
/**
 * Returns true if currently being executed from inside the Picker monorepo.
 */
export function isInPickerMonorepo(): boolean {
...
/**
 * Copy the @picker/ui-devkit files to the static assets dir.
 */
export function copyUiDevkit() {
    const devkitDir = path.join(STATIC_ASSETS_OUTPUT_DIR, 'devkit');
    fs.ensureDirSync(devkitDir);
    fs.copySync(require.resolve('@picker-cc/ui-devkit'), path.join(devkitDir, 'ui-devkit.js'));
}
