...
module.exports = (eleventyConfig) => {
    ...
    markdownIt.renderer.rules.image = (tokens) => {
        // Sometimes this retuns things that arn’t images so we need to filters
        try {
    ...
    eleventyConfig.addPairedShortcode(
        "memberInfo",
        (content, kind, typeCode, defaultCode) => {
    ...
    eleventyConfig.addPairedShortcode(
        "generationInfo",
        (content, sourceFile, sourceLine, packageName) => {
            let index = sourceFile.lastIndexOf('\/');
            let sourceFileName = sourceFile.substring(index + 1, sourceFile.length);
            return `<div class="generation-info">
              <span class="label">Package:</span>
              <a href="https://www.npmjs.com/package/${packageName}">
               ${packageName}
              </a>
              <span class="label">File:</span>
              <a href="https://github.com/picker-cc/picker/blob/master/${sourceFile}#${sourceLine}">
                ${sourceFileName}
              </a>
            </div>`
        }
    ...
    eleventyConfig.addPairedShortcode(
        "memberDescription",
        (content) => {
            return `<div class="member-description">${content}</div>`
        }
    );
